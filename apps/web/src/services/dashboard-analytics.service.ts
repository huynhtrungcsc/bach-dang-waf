...
export const dashboardAnalyticsService = {
   ...
   */
  getLatestAttackStats: (limit: number = 5, hours: number = 24) =>
    fetchAnalytics<AttackTypeStats[]>('latest-attacks', { limit, hours }),

  /**
   * Get latest security news/events
   */
  getLatestNews: (limit: number = 20, hours: number = 24) =>
    fetchAnalytics<LatestAttackEntry[]>('latest-news', { limit, hours }),
   ...
   */
  getGeoStats: (mode: 'requests' | 'blocked' = 'requests') =>
    fetchAnalytics<Array<{ name: string; value: number }>>('geo-stats', { mode }),
...
export const getGeoStats = dashboardAnalyticsService.getGeoStats;
