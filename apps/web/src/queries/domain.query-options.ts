...
export const useCreateDomain = () => {
  ...
  return useMutation({
    ...
    onSuccess: async (data: Domain) => {
      ...
      await queryClient.refetchQueries({ queryKey: domainQueryKeys.all });
    },
  });
};
...
export const useUpdateDomain = () => {
  ...
  return useMutation({
    ...
    onSuccess: async (data: Domain, { id }) => {
...
export const useToggleDomainSSL = () => {
  ...
  return useMutation({
    ...
    onMutate: async ({ id, sslEnabled }: { id: string; sslEnabled: boolean }) => {
      const detailKey = domainQueryKeys.detail(id);
      await queryClient.cancelQueries({ queryKey: detailKey });
      const previous = queryClient.getQueryData<Domain>(detailKey);
      if (previous) {
        queryClient.setQueryData<Domain>(detailKey, { ...previous, sslEnabled });
      }
