...
export const dashboardAnalyticsQueryOptions = {
  ...
  latestAttacks: (limit: number = 5, hours: number = 24) => ({
    // hours param drives both cache key and API query
    queryKey: dashboardAnalyticsQueryKeys.list({ type: 'attacks', limit, hours }),
    queryFn: () => dashboardAnalyticsService.getLatestAttackStats(limit, hours),
    refetchInterval: 10 * 1000, // Auto-refresh every 10 seconds
  }),

  // Get latest security news/events
  latestNews: (limit: number = 20, hours: number = 24) => ({
    // hours param drives both cache key and API query
    queryKey: dashboardAnalyticsQueryKeys.list({ type: 'news', limit, hours }),
    queryFn: () => dashboardAnalyticsService.getLatestNews(limit, hours),
    refetchInterval: 10 * 1000, // Auto-refresh every 10 seconds
  }),
  ...
  // Get geo location stats (country → count)
  geoStats: (mode: 'requests' | 'blocked' = 'requests') => ({
    queryKey: dashboardAnalyticsQueryKeys.list({ type: 'geoStats', mode }),
    queryFn: () => dashboardAnalyticsService.getGeoStats(mode),
...
export const useLatestAttackStats = (limit: number = 5, hours: number = 24) => {
  return useQuery(dashboardAnalyticsQueryOptions.latestAttacks(limit, hours));
};

export const useLatestNews = (limit: number = 20, hours: number = 24) => {
  return useQuery(dashboardAnalyticsQueryOptions.latestNews(limit, hours));
};
...
export const useGeoStats = (mode: 'requests' | 'blocked' = 'requests') => {
  return useQuery(dashboardAnalyticsQueryOptions.geoStats(mode));
};
...
export const useSuspenseLatestAttackStats = (limit: number = 5, hours: number = 24) => {
  return useSuspenseQuery(dashboardAnalyticsQueryOptions.latestAttacks(limit, hours));
};

export const useSuspenseLatestNews = (limit: number = 20, hours: number = 24) => {
  return useSuspenseQuery(dashboardAnalyticsQueryOptions.latestNews(limit, hours));
};
