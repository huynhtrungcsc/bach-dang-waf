...
export const useCreateAclRule = () => {
  ...
  return useMutation({
    ...
    onSuccess: async (data: ACLRule) => {
      aclMutationOptions.create.onSuccess?.(data);
      await queryClient.invalidateQueries({ queryKey: aclQueryKeys.all });
      await queryClient.refetchQueries({ queryKey: aclQueryKeys.all });
    },
  });
};
...
export const useUpdateAclRule = () => {
  ...
  return useMutation({
    ...
    onSuccess: async (data: ACLRule, { id }) => {
...
export const useImportAclRules = () => {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: ({ rules, mode }: { rules: any[]; mode: 'merge' | 'replace' }) =>
      aclService.importRules(rules, mode),
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: aclQueryKeys.all });
      await queryClient.refetchQueries({ queryKey: aclQueryKeys.all });
    },
