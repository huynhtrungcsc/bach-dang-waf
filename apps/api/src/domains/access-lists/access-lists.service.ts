...
export class AccessListsService {
  async getAccessLists(
    ...
    return accessListsRepository.listAll(options);
  }
  async getAccessListById(id: string): Promise<AccessListWithRelations | null> {
    return accessListsRepository.getById(id);
  }
  async createAccessList(
    ...
    const existing = await accessListsRepository.getByName(input.name);
    if (existing) {
    ...
    // Create access list
    const accessList = await accessListsRepository.insert(input);
  ...
  async updateAccessList(
    ...
    const existing = await accessListsRepository.getById(id);
    if (!existing) {
    ...
    if (input.name && input.name !== existing.name) {
      const duplicate = await accessListsRepository.getByName(input.name);
      if (duplicate) {
        throw new Error('Access list with this name already exists');
      }
    }
    ...
    // Update access list
    const accessList = await accessListsRepository.patch(id, input);
    ...
    if (accessList.domains && accessList.domains.length > 0) {
      for (const domainLink of accessList.domains) {
        try {
          await domainsService.rebuildConfig(domainLink.domainId);
  ...
  async deleteAccessList(
    ...
    // Check if access list exists
    const accessList = await accessListsRepository.getById(id);
    if (!accessList) {
      throw new Error('Access list not found');
    }
    ...
    await accessListsRepository.remove(id);
  ...
  async toggleAccessList(
    ...
    const accessList = await accessListsRepository.setEnabled(id, enabled);
  ...
  async applyToDomain(
    ...
    const accessList = await accessListsRepository.getById(accessListId);
    if (!accessList) {
      throw new Error('Access list not found');
    }
    ...
    await domainsService.rebuildConfig(domainId);
  ...
  async getAccessListsByDomainId(domainId: string): Promise<AccessListWithRelations[]> {
    return accessListsRepository.getByDomainId(domainId);
  }
  async getStats() {
