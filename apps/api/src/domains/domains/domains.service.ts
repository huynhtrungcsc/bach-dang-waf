...
export class DomainsService {
  async listDomains(
    ...
    return domainsRepository.listAll(options);
  }
  async findDomain(id: string): Promise<DomainWithRelations | null> {
  ...
  async addDomain(
    ...
    const existingDomain = await domainsRepository.getByName(input.name);
    if (existingDomain) {
      throw new Error('Domain already exists');
    }

    // Create domain
    const domain = await domainsRepository.insert(input);
    ...
    try {
      ...
      if (!reloadResult.success) {
        ...
        await domainsRepository.remove(domain.id);
      ...
      if (input.autoCreateSSL && input.sslEmail) {
        try {
          ...
          try {
            ...
            const enabledDomain = await domainsRepository.patch(updatedDomain.id, {
              sslEnabled: true,
  ...
  async rebuildConfig(domainId: string): Promise<void> {
    // Get domain with all relations including access lists
    const domain = await domainsRepository.getById(domainId);
    if (!domain) {
  ...
  async editDomain(
    ...
    const originalDomain = await domainsRepository.getById(id);
    if (!originalDomain) {
      throw new Error('Domain not found');
    }
    ...
    try {
      // Update domain
      await domainsRepository.patch(id, input);

      // Get updated domain with relations
      const updatedDomain = await domainsRepository.getById(id);
      if (!updatedDomain) {
      ...
      if (!reloadResult.success) {
        ...
        await domainsRepository.patch(id, originalData);
        const restoredDomain = await domainsRepository.getById(id);
        if (restoredDomain) {
  ...
  async removeDomain(
    ...
    // Check if domain exists
    const domain = await domainsRepository.getById(id);
    if (!domain) {
      throw new Error('Domain not found');
    }
    ...
    await domainsRepository.remove(id);
