...
export class DomainsController {
  async listDomains(req: AuthRequest, res: Response): Promise<void> {
    try {
      const {
        page = 1,
        limit = 10,
      ...
      const result = await domainsService.listDomains(options);
  ...
  async findDomain(req: AuthRequest, res: Response): Promise<void> {
    try {
      const { id } = req.params;

      const domain = await domainsService.findDomain(id);
  ...
  async addDomain(req: AuthRequest, res: Response): Promise<void> {
    try {
  ...
  async editDomain(req: AuthRequest, res: Response): Promise<void> {
    try {
      const errors = validationResult(req);
      if (!errors.isEmpty()) {
        res.status(400).json({
      ...
      const domain = await domainsService.editDomain(
        id,
        {
          name,
          status,
  ...
  async removeDomain(req: AuthRequest, res: Response): Promise<void> {
    try {
      const { id } = req.params;

      await domainsService.removeDomain(
        id,
        req.user!.userId,
        req.user!.username,
        req.ip || 'unknown',
  ...
  async switchSSL(req: AuthRequest, res: Response): Promise<void> {
    try {
      const { id } = req.params;
      ...
      const domain = await domainsService.switchSSL(
        id,
        sslEnabled,
        req.user!.userId,
        req.user!.username,
  ...
  async reloadProxy(req: AuthRequest, res: Response): Promise<void> {
    try {
      const result = await domainsService.reloadProxy(
        req.user!.userId,
        req.user!.username,
        req.ip || 'unknown',
        req.headers['user-agent'] || 'unknown'
