...
export class DomainsRepository {
   ...
   * Find domain by ID
   */
  async getById(id: string): Promise<DomainWithRelations | null> {
   ...
   * Find domain by name
   */
  async getByName(name: string): Promise<DomainWithRelations | null> {
    const domain = await prisma.domain.findUnique({
      where: { name },
      include: {
        upstreams: true,
   ...
   * Create new domain
   */
  async insert(input: CreateDomainInput): Promise<DomainWithRelations> {
   ...
   * Update domain
   */
  async patch(id: string, input: UpdateDomainInput): Promise<DomainWithRelations> {
    // Get current domain
    ...
    return this.getById(id) as Promise<DomainWithRelations>;
  }
