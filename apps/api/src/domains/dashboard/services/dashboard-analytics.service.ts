...
const NGINX_ACCESS_LOG = NGINX_LOG_DIR + '/access.log';
const NGINX_ERROR_LOG = NGINX_LOG_DIR + '/error.log';
const MODSEC_AUDIT_LOG = process.env.MODSEC_AUDIT_LOG || '/var/log/modsec_audit.log';
...
export class DashboardAnalyticsService {
  ...
  async getLatestAttacks(limit: number = 5, hours: number = 24): Promise<AttackTypeStats[]> {
    try {
      // Scale line buffer with time window
      const lineBuffer = hours <= 1 ? 1000 : hours <= 6 ? 2000 : hours <= 24 ? 5000 : 20000;
      const lines = await this.readModSecLogs(lineBuffer);
      ...
      const cutoffTime = this.getCutoffTime(hours);
  ...
  async getLatestNews(limit: number = 20, hours: number = 24): Promise<LatestAttackEntry[]> {
    try {
      // Scale line buffer with time window
      const lineBuffer = hours <= 1 ? 500 : hours <= 6 ? 1000 : hours <= 24 ? 2000 : 8000;
      const lines = await this.readModSecLogs(lineBuffer);
  ...
  async getGeoStats(mode: 'requests' | 'blocked' = 'requests'): Promise<Array<{ name: string; value: number }>> {
    try {
      ...
      if (mode === 'blocked') {
        const lines = await this.readModSecLogs(20000);
        const cutoff = this.getCutoffTime(24);
        lines.forEach((line, idx) => {
          const parsed = parseModSecLogLine(line, idx);
          if (!parsed?.ip) return;
          if (new Date(parsed.timestamp).getTime() < cutoff) return;
          const geo = geoip.lookup(parsed.ip);
          if (!geo?.country) return;
          try { const n = dn.of(geo.country) ?? geo.country; countryMap.set(n, (countryMap.get(n) ?? 0) + 1); } catch {}
        });
      } else {
        const lines = await this.readAllAccessLogs(50000, 20000);
        const cutoff = this.getCutoffTime(24);
        for (const line of lines) {
          const parsed = parseAccessLogLine(line, 0);
          if (!parsed?.ip) continue;
          if (new Date(parsed.timestamp).getTime() < cutoff) continue;
          const geo = geoip.lookup(parsed.ip);
