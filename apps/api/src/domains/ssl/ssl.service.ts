...
export class SSLService {
  ...
  async listCerts(): Promise<SSLCertificateWithStatus[]> {
      ...
      certificates.map(async (cert) => {
        ...
        try {
          ...
          if (
            ...
            await sslRepository.patch(cert.id, updateData);
          }
        } catch (error) {
  ...
  async findCert(id: string): Promise<SSLCertificateWithDomain | null> {
    return sslRepository.getById(id);
  }
  async issueAutoSSL(
    dto: IssueAutoSSLDto,
    userId: string,
    ...
    const existingCert = await sslRepository.getByDomainId(domainId);
    if (existingCert) {
    ...
    try {
      ...
      const sslCertificate = await sslRepository.insert(createData);
  ...
  async uploadManualSSL(
    ...
    const cert = await sslRepository.insert(createData);
  ...
  async editCert(
    ...
    const cert = await sslRepository.getById(id);
    if (!cert) {
    ...
    // Update certificate
    const updatedCert = await sslRepository.patch(id, updateData);
  ...
  async renewCert(
    ...
    try {
      ...
      const certFiles = await acmeService.renewCert(cert.domain.name);
