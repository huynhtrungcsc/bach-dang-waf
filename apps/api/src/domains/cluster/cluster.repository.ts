...
import { ReplicaNode, ReplicaNodeResponse, SyncConfigData } from './cluster.types';

/**
 * Cluster Repository - Database operations for replica nodes
 */
export class ClusterRepository {
  /**
   * Find replica node by name
   */
  async getByName(name: string): Promise<ReplicaNode | null> {
    return prisma.replicaNode.findUnique({
      where: { name }
    });
  }

  /**
   * Find replica node by ID
   */
  async getById(id: string): Promise<ReplicaNodeResponse | null> {
    return prisma.replicaNode.findUnique({
      where: { id },
      select: {
        id: true,
   ...
   */
  async findByApiKey(apiKey: string): Promise<Pick<ReplicaNode, 'id' | 'name' | 'host' | 'port' | 'syncEnabled'> | null> {
    return prisma.replicaNode.findFirst({
      where: { apiKey },
      select: {
        id: true,
        name: true,
  ...
  async insert(data: {
    ...
    return prisma.replicaNode.create({
      data: {
        ...data,
  ...
  /**
   * Get all replica nodes (without API keys)
   */
  async listAll(): Promise<ReplicaNodeResponse[]> {
    return prisma.replicaNode.findMany({
      orderBy: {
        createdAt: 'desc'
      },
      select: {
  ...
  async remove(id: string): Promise<void> {
    await prisma.replicaNode.delete({
      where: { id }
    });
  }

  /**
   * Update replica node fields (name, host, port, syncInterval, syncEnabled)
   */
  async patch(id: string, data: {
    ...
    const updated = await prisma.replicaNode.update({
      where: { id },
      data,
      select: {
        id: true,
  ...
  /**
   * Update replica node last seen timestamp
   */
  async updateLastSeen(id: string, lastSeen: Date = new Date()): Promise<void> {
    await prisma.replicaNode.update({
      where: { id },
      data: { lastSeen }
  ...
  /**
   * Update replica node last seen timestamp and status
   */
  ...
  /**
   * Update replica node config hash
   */
  ...
  async markNodesOffline(nodeIds: string[]): Promise<void> {
    await prisma.replicaNode.updateMany({
      where: {
        id: {
          in: nodeIds
        }
