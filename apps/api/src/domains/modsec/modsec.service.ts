...
export class ModSecService {
  ...
  private async regenerateCRSDisableConfig(domainId?: string): Promise<void> {
    try {
      ...
      disableContent += '# Auto-generated by BACH DANG WAF - DO NOT EDIT MANUALLY\n';
      ...
      // Write to single disable file (skip gracefully if path doesn't exist in dev/non-production)
      try {
        await fs.mkdir(path.dirname(MODSEC_CRS_DISABLE_FILE), { recursive: true });
        ...
        logger.warn(`Skipping CRS disable file write (path not available): ${fsError.message}`);
      }
    } catch (error) {
  ...
  async switchCrsRule(ruleFile: string, dto: ToggleCRSRuleDto): Promise<CRSRule> {
    const { domainId } = dto;
  ...
  async listCustomRules(domainId?: string): Promise<ModSecRule[]> {
    return modSecRepository.findModSecRules(domainId);
  }

  async findCustomRule(id: string): Promise<ModSecRuleWithDomain> {
  ...
  async switchCustomRule(id: string): Promise<ModSecRule> {
    const rule = await modSecRepository.findModSecRuleById(id);
    if (!rule) {
      throw new Error('ModSecurity rule not found');
    }

    const updatedRule = await modSecRepository.switchCustomRule(id, !rule.enabled);
  ...
  async addRule(dto: AddCustomRuleDto): Promise<ModSecRule> {
      ...
      if (rule.enabled) {
        ...
        if (!configTest.valid) {
          ...
          try {
            ...
            await modSecRepository.removeRule(rule.id);
          } catch (rollbackError) {
  ...
  async editRule(id: string, dto: UpdateModSecRuleDto): Promise<ModSecRule> {
    const rule = await modSecRepository.findModSecRuleById(id);
    if (!rule) {
      throw new Error('ModSecurity rule not found');
    }
    ...
    const updatedRule = await modSecRepository.editRule(id, dto);
        ...
        if (!configTest.valid) {
          ...
          if (backupFilePath && currentFilePath) {
            try {
              ...
              await modSecRepository.editRule(id, {
  ...
  async removeRule(id: string): Promise<void> {
    ...
    await modSecRepository.removeRule(id);
  ...
  async getGlobalSettings(): Promise<GlobalModSecSettings> {
  ...
  async setGlobalSettings(dto: SetGlobalModSecDto) {
    const { enabled } = dto;
