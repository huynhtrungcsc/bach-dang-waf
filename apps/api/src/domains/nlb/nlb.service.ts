...
import { WafException } from '../../middleware/errorHandler';
...
export class NLBService {
  ...
  async listNlbs(
    ...
    return this.repository.listAll(options);
  }
  async findNlb(id: string): Promise<NLBWithRelations> {
    const nlb = await this.repository.getById(id);
    if (!nlb) {
      throw new WafException('NLB not found', 404);
    }
    return nlb;
  }
  async addNlb(input: CreateNLBInput): Promise<NLBWithRelations> {
    // Validate port range
    if (input.port < 10000) {
      throw new WafException('NLB port must be 10000 or higher', 400);
    }

    // Check if name already exists
    const existingByName = await this.repository.getByName(input.name);
    if (existingByName) {
      throw new WafException(`NLB with name "${input.name}" already exists`, 409);
    }

    // Check if port already in use
    const existingByPort = await this.repository.findByPort(input.port);
    if (existingByPort) {
      throw new WafException(`Port ${input.port} is already in use by NLB "${existingByPort.name}"`, 409);
    }

    // Create NLB in database
    const nlb = await this.repository.insert(input);
    ...
    try {
      ...
      return this.repository.getById(nlb.id) as Promise<NLBWithRelations>;
    } catch (error) {
      logger.error(`Failed to create NLB config for ${nlb.name}:`, error);
      // Rollback: delete the DB record so retry with same name is allowed
      try { await this.repository.remove(nlb.id); } catch (_) {}
      throw new WafException('Failed to create NLB configuration. Nginx may not be available.', 500);
    }
  }
  async editNlb(id: string, input: UpdateNLBInput): Promise<NLBWithRelations> {
    const nlb = await this.findNlb(id);
    ...
    const updatedNLB = await this.repository.patch(id, input);
    ...
    if (updatedNLB.enabled) {
      try {
        ...
        throw new WafException('Failed to update NLB configuration', 500);
      }
    } else {
    ...
    return this.repository.getById(id) as Promise<NLBWithRelations>;
  }
  async removeNlb(id: string): Promise<void> {
    const nlb = await this.findNlb(id);

    // Remove Nginx configuration (best-effort — don't block delete if nginx unavailable)
    try {
      await this.disableStreamConfig(nlb.name);
      ...
      logger.warn(`Nginx cleanup failed for NLB ${nlb.name} (proceeding with DB delete): ${nginxErr.message}`);
    }
  ...
  async switchNlb(id: string, enabled: boolean): Promise<NLBWithRelations> {
    const nlb = await this.findNlb(id);
    ...
    if (enabled) {
      ...
      await this.testNginxConfig();
      await this.reloadNginx();
      await this.repository.setEnabled(id, true);
      await this.repository.updateStatus(id, 'active');
...
# DO NOT EDIT THIS FILE MANUALLY - Managed by BACH DANG WAF
...
`;
  ...
  private async ensureStreamInclude(): Promise<void> {
    try {
      ...
      throw new WafException('Failed to create stream directories', 500);
    }
  }
  private async testNginxConfig(): Promise<void> {
    try {
      ...
      throw new WafException(`Nginx configuration test failed: ${error.message}`, 500);
    }
  }
  private async reloadNginx(): Promise<void> {
    try {
      ...
      throw new WafException(`Failed to reload Nginx: ${error.message}`, 500);
    }
  }
