...
export class NLBRepository {
   ...
   * Find NLB by ID
   */
  async getById(id: string): Promise<NLBWithRelations | null> {
   ...
   * Find NLB by name
   */
  async getByName(name: string): Promise<NLBWithRelations | null> {
    const nlb = await prisma.networkLoadBalancer.findUnique({
      where: { name },
      include: {
        upstreams: true,
   ...
   * Create new NLB
   */
  async insert(input: CreateNLBInput): Promise<NLBWithRelations> {
   ...
   * Update NLB
   */
  async patch(id: string, input: UpdateNLBInput): Promise<NLBWithRelations> {
   ...
   */
  async setEnabled(id: string, enabled: boolean): Promise<NLBWithRelations> {
    const nlb = await prisma.networkLoadBalancer.update({
      where: { id },
      data: { 
        enabled,
