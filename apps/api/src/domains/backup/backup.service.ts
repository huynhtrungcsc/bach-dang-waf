...
export class BackupService {
  ...
  async listSchedules(): Promise<FormattedBackupSchedule[]> {
  ...
  async addSchedule(dto: CreateBackupScheduleDto, userId?: string) {
  ...
  async switchSchedule(id: string, userId?: string) {
    const schedule = await backupRepository.findScheduleById(id);
    if (!schedule) {
      throw new Error('Backup schedule not found');
    }
  ...
  async importBackup(backupData: any, userId?: string) {
  ...
  async listBackupFiles(scheduleId?: string): Promise<FormattedBackupFile[]> {
  ...
  async findBackupFile(id: string) {
  ...
  async removeBackupFile(id: string, userId?: string) {
    const backup = await backupRepository.findBackupFileById(id);
    if (!backup) {
      throw new Error('Backup file not found');
    }
    ...
    await backupRepository.removeBackupFile(id);
  ...
  private async collectBackupData(): Promise<BackupData> {
    ...
    // Get users
    const users = await backupRepository.listUsers();
