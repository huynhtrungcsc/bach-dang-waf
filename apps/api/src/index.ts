...
import { startReplicaStatusCheck, stopReplicaStatusCheck } from './domains/cluster/services/replica-status.service';
...
let replicaStatusTimer: NodeJS.Timeout | null = null;
...
// In development, proxy all non-API requests to the Vite frontend dev server (port 5000)
...
if (config.nodeEnv === 'development') {
  app.use(createProxyMiddleware({
    ...
    on: {
      error: (_err, _req, res) => {
        (res as any).status?.(502).json({ success: false, message: 'Frontend dev server not ready yet' });
      },
    },
  }));
} else {
...
const server = app.listen(PORT, async () => {
  ...
  // Start replica node status checker (check every minute)
  replicaStatusTimer = startReplicaStatusCheck();
...
process.on('SIGTERM', () => {
  ...
  if (replicaStatusTimer) {
    stopReplicaStatusCheck(replicaStatusTimer);
  }
